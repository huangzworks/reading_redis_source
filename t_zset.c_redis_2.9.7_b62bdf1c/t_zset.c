...
typedef struct zskiplistNode {              // 跳跃表节点
    robj *obj;
    double score;                           // 分值
    struct zskiplistNode *backward;         // 双链表的向后指针
    struct zskiplistLevel {                 // 层结构
        struct zskiplistNode *forward;      // 层结构向前指针
        unsigned int span;
    } level[];                              
} zskiplistNode;                            
...
// 返回一个随机数，作为新见层的层数
//
// 返回值是一个大于等于 1 ，且小于等于 ZSKIPLIST_MAXLEVEL 的值
// 根据 powerlaw 分布，越大的随机数返回的几率越小
...
zskiplistNode *zslInsert(zskiplist *zsl, double score, robj *obj) {
    ...
    // 如果新节点的 level 比 zsl->level 要大
    // 那么更新 zsl->header 的指针，将它们指向新节点
    if (level > zsl->level) {
...
// 从跳跃表中删除一个匹配给定 score 和给定 obj 的元素
int zslDelete(zskiplist *zsl, double score, robj *obj) {
    ...
    // 因为这个跳跃表实现允许有多个同样 score 值的元素
    // 因此在找到给定 score 值的元素之后，还要检查元素是否就是要删除的 obj
    x = x->level[0].forward;
...
// 检查给定 value 是否大于/大于等于指定区间的最小值
static int zslValueGteMin(double value, zrangespec *spec) {
    return spec->minex ? (value > spec->min) : (value >= spec->min);
}

// 检查给定 value 是否小于/小于等于指定区间的最大值
static int zslValueLteMax(double value, zrangespec *spec) {
    return spec->maxex ? (value < spec->max) : (value <= spec->max);
}

/* Returns if there is a part of the zset is in range. */
// 检查 zsl 是否包含给定 score 区间值
...
unsigned long zslDeleteRangeByScore(zskiplist *zsl, zrangespec range, dict *dict) {
    ...
    while (x && (range.maxex ? x->score < range.max : x->score <= range.max)) {
        zskiplistNode *next = x->level[0].forward;  // 后继指针
        zslDeleteNode(zsl,x,update);    // 在跳跃表中删除
        dictDelete(dict,x->obj);    // 同时在字典中删除
        zslFreeNode(x);
        removed++;
        x = next;   // 调整指针
    }
    return removed; // 返回被移除节点的数量
}
 ...
 * first element. */
// 根据给定 score 和 obj ，返回该元素在 zsl 中的 rank
...
void zaddGenericCommand(redisClient *c, int incr) {
    ...
    // 查找给定 key ，如果不存在，就创建一个 sorted set
    ...
    // 将所有 elements 加入到 sorted set （可能是 ZIPLIST 或 ZSET）
    for (j = 0; j < elements; j++) {
        score = scores[j];
        ...
        if (zobj->encoding == REDIS_ENCODING_ZIPLIST) {
            ...
            de = dictFind(zs->dict,ele);    // 元素是否已经存在？
            if (de != NULL) {
                // 修改已有节点
